...
export default async function SupportPage() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case "in_progress": return <Badge variant="outline" className="text-blue-600 bg-blue-50 border-blue-200">In Progress</Badge>;
      case "resolved": return <Badge variant="outline" className="text-emerald-600 bg-emerald-50 border-emerald-200">Resolved</Badge>;
    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl space-y-12">
      <div className="text-center md:text-left space-y-4">
        ...
        <p className="text-lg text-slate-500 max-w-2xl font-medium leading-relaxed opacity-80">
          Need help or want to suggest an improvement? We&apos;re here to listen and help you succeed.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
        ...
        <div className="lg:col-span-3 order-1 lg:order-2 space-y-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge className="bg-slate-100 text-slate-500 shadow-none border-0 font-black">{feedbacks.length} Total</Badge>
          </div>

          <div className="space-y-4 max-h-[600px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-slate-200">
            {feedbacks.length > 0 ? (
              feedbacks.map((item) => (
                <Card key={item.id} className="border-0 shadow-lg bg-white overflow-hidden hover:shadow-xl transition-all">
                  <CardContent className="p-6">
                    ...
                    <h3 className="text-lg font-black text-slate-900 mb-2">{item.subject}</h3>
              ...
              <div className="text-center py-20 bg-slate-50 rounded-[2.5rem] border-2 border-dashed border-slate-200">
                ...
                <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">No support requests yet</p>
              </div>
            )}
          </div>
        </div>
