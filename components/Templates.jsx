...
import { Mail, Phone, Linkedin, Globe, MapPin, GraduationCap, Briefcase, Code, Award } from 'lucide-react';

const Template = ({ data, activeStyle, mode, letterData }) => {
  ...
  if (mode === 'letter') {
      ...
      return (
        <div id="resume-ats" className="bg-white p-[45px_55px] max-w-[210mm] mx-auto min-h-[297mm] text-[#374151] leading-[1.4] font-sans text-[10pt]">
          <header className="mb-5">
            <h1 className="text-[26px] font-bold text-[#111827] tracking-tight">{contactInfo?.name}</h1>
    ...
    const renderAcademic = () => {
      ...
      return (
        <div
          id="resume-academic"
          className="bg-white p-[35px_45px] max-w-[210mm] mx-auto min-h-[297mm] text-[#334155] leading-normal font-sans text-[10.5pt]"
        >
          ...
          {summary && (
            <section className="mb-2">
              ...
              <p className="text-justify text-[12.5px] text-[#475569] font-normal m-0">
                {summary}
              </p>
            </section>
          )}
            ...
            {education?.map((edu, i) => (
              <div key={i} className="mb-2">
                <div className="flex justify-between font-semibold text-[#1e293b]">
                  ...
                  <span className="text-[#4f46e5] text-[10px]">
                    {edu?.startDate} – {edu?.endDate}
                  </span>
                </div>
                <div className="text-[#64748b] italic text-[12px]">
                  {edu?.title}
                </div>
              </div>
            ))}
            ...
            {experience?.map((exp, i) => (
              <div key={i} className="mb-3">
                <div className="flex justify-between font-semibold text-[#1e293b]">
                  ...
                  <span className="text-[9px] bg-[#f0f4ff] text-[#4f46e5] px-1.5 py-0.5 rounded">
                    {exp?.startDate} – {exp?.endDate}
                  </span>
                </div>
                <div className="text-[#4f46e5] uppercase text-[10px] font-bold mb-1 tracking-wider">
                  {exp?.title}
                </div>
                <p className="text-[12.5px] border-l-2 border-[#e2e8f0] pl-3 text-justify">
                  {exp?.description}
                </p>
              </div>
            ))}
              ...
              <div key={i} className="mb-2">
                <div className="flex justify-between items-baseline">
                  ...
                  <div className="flex gap-2 text-[9px] font-bold text-[#4f46e5]">
                    {proj?.githubLink && (
                      <a href={formatUrl(proj.githubLink)}>GitHub</a>
                    )}
                    {proj?.liveLink && (
                      <a href={formatUrl(proj.liveLink)}>Live</a>
                    )}
                  </div>
                </div>
          ...
          <section className="mb-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              {userSkills.map((skill, i) => (
                <span
                  key={i}
                  className="px-2.5 py-1 rounded-lg text-[10px] border border-[#e2e8f0] font-semibold text-[#334155] uppercase"
                >
                  {skill.trim()}
                </span>
              ))}
      ...
      return (
        ...
        >
          <header className="mb-6 border-l-8 border-[#4f46e5] pl-5">
            <h1 className="text-[24px] font-sans font-black tracking-tighter uppercase text-[#0f172a]">
              {contactInfo?.name}
            </h1>
            <p className="text-[14px] font-bold text-[#4f46e5] uppercase mb-2">
              {contactInfo?.profession}
            </p>
          ...
          <section className="mb-4">
            ...
            {experience?.map((exp, i) => (
              <div key={i} className="mb-4">
                <div className="flex justify-between items-baseline">
                  <span className="text-[13.5px] font-bold text-[#0f172a] underline decoration-[#cbd5e1] underline-offset-4">
                    {exp?.organization}
                  </span>
                  <span className="text-[9px] font-bold text-[#4f46e5] uppercase">
                    {exp?.startDate} – {exp?.endDate}
                  </span>
                </div>
                <div className="text-[#64748b] text-[10px] font-bold uppercase italic mb-1">
                  {exp?.title}
                </div>
                <p className="text-[12.5px] border-l-2 border-[#f1f5f9] pl-3">
                  {exp?.description}
                </p>
              </div>
            ))}
          ...
          <section className="mb-4">
            ...
            {projects?.map((proj, i) => (
              <div
                key={i}
                className="mb-3 border-l-2 border-[#4f46e5] pl-3"
              >
            ...
            {education?.map((edu, i) => (
              <div key={i} className="flex justify-between mb-2">
                <div>
                  ...
                  <div className="text-[12px] italic">{edu?.title}</div>
                </div>
          ...
          <section className="mb-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {userSkills.map((s, i) => (
                <span
                  key={i}
                  className="px-2.5 py-1 text-[10px] bg-[#1e293b] font-bold text-white uppercase"
                >
                  {s.trim()}
                </span>
              ))}
    ...
    const renderExecutive = () => {
      ...
      return (
        <div
          id="resume-executive"
          className="bg-white p-[35px_45px] max-w-[210mm] mx-auto min-h-[297mm] text-[#3f3f46] font-serif text-[10.5pt]"
        >
          <header className="text-center mb-8">
            <h1 className="text-[28px] font-light tracking-tight text-[#18181b]">
              {contactInfo?.name}
            </h1>
            <div className="text-[11px] font-sans uppercase tracking-[0.3em] text-[#a1a1aa] mb-3">
              {contactInfo?.profession}
            </div>
            ...
            {experience?.map((exp, i) => (
              <div key={i} className="mb-5">
                <div className="flex justify-between items-end mb-1">
                  ...
                  <div className="text-[10px] font-sans font-bold text-[#a16207] tracking-widest">
                    {exp?.startDate} — {exp?.endDate}
                  </div>
                </div>
                <div className="text-[12px] text-[#71717a] font-sans font-semibold uppercase mb-2">
                  {exp?.organization}
                </div>
                <p className="text-[12.5px] text-justify leading-relaxed">
                  {exp?.description}
                </p>
              </div>
            ))}
            ...
            {projects?.map((proj, i) => (
              <div key={i} className="mb-4">
                <div className="flex justify-between items-center mb-1">
                  <span className="font-bold text-[#18181b] italic">
                    {proj?.title}
                  </span>
                  <div className="flex gap-3 text-[9px] font-sans font-bold text-[#a16207] uppercase">
                    {proj?.githubLink && (
                      <a href={formatUrl(proj.githubLink)}>Repo</a>
                    )}
                    {proj?.liveLink && (
                      <a href={formatUrl(proj.liveLink)}>Live</a>
                    )}
                ...
                <p className="text-[12px] italic pr-4 border-r-2 border-[#f4f4f5] text-right">
                  {proj?.description}
                </p>
              </div>
            ))}
            ...
            <div className="grid grid-cols-2 gap-4">
              {education?.map((edu, i) => (
                <div key={i} className="border-l border-[#d4d4d8] pl-4">
                  <div className="font-bold text-[#18181b] text-[13px]">
                    {edu?.organization}
                  </div>
                  <div className="text-[11px] italic text-[#a16207]">
                    {edu?.title}
                  </div>
                  <div className="text-[9px] font-sans font-bold text-[#a1a1aa] mt-1">
                    {edu?.startDate} — {edu?.endDate}
                  </div>
                </div>
              ))}
    ...
    // --- TEMPLATE 5: MODERN SIDEBAR ---
    const renderModern = () => {
      const sectionHeader = "text-[15px] font-black text-[#0f172a] mb-5 uppercase tracking-[0.15em] flex items-center gap-3 after:content-[''] after:h-[1px] after:flex-1 after:bg-slate-100";
      const sidebarSectionHeader = "text-[11px] font-black text-indigo-400 mb-4 uppercase tracking-[0.25em]";

      return (
        <div
          id="resume-modern"
          className="bg-white max-w-[210mm] mx-auto min-h-[297mm] flex text-[#334155] font-sans shadow-xl overflow-hidden"
        >
          <aside className="w-[32%] bg-[#0f172a] text-white p-7 flex flex-col gap-8 border-r border-slate-800">
            <div>
              <h2 className={sidebarSectionHeader}>Contact</h2>
              <div className="flex flex-col gap-4 text-[11px]">
                {contactInfo?.email && (
                  <div className="flex flex-col">
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Email</span>
                    <a href={`mailto:${contactInfo.email}`} className="text-slate-300 hover:text-white truncate font-medium transition-colors">{contactInfo.email}</a>
                  </div>
                )}
                {contactInfo?.mobile && (
                  <div className="flex flex-col">
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Phone</span>
                    <a href={`tel:${contactInfo.mobile}`} className="text-slate-300 hover:text-white font-medium transition-colors">{contactInfo.mobile}</a>
                  </div>
                )}
                {contactInfo?.linkedin && (
                  <div className="flex flex-col">
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">LinkedIn</span>
                    <a href={formatUrl(contactInfo.linkedin)} target="_blank" rel="noopener noreferrer" className="text-slate-300 hover:text-white truncate font-medium transition-colors">linkedin.com/in</a>
                  </div>
                )}
                {contactInfo?.portfolio && (
                  <div className="flex flex-col">
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Portfolio</span>
                    <a href={formatUrl(contactInfo.portfolio)} target="_blank" rel="noopener noreferrer" className="text-slate-300 hover:text-white truncate font-medium transition-colors">Portfolio Website</a>
                  </div>
                )}
              </div>
            </div>
            ...
            <div>
              <h2 className={sidebarSectionHeader}>Education</h2>
              <div className="flex flex-col gap-5">
                {education?.map((edu, i) => (
                  <div key={i} className="flex flex-col gap-1 group">
                    <div className="text-[12px] font-bold text-white leading-tight group-hover:text-indigo-400 transition-colors italic">{edu?.organization}</div>
                    <div className="text-[10px] text-slate-400 font-medium">{edu?.title}</div>
            ...
            <div>
              <h2 className={sidebarSectionHeader}>Expertise</h2>
              <div className="flex flex-wrap gap-1.5">
                {userSkills.map((skill, i) => (
                  <span
                    key={i}
                    className="px-2 py-0.5 bg-slate-800/50 hover:bg-indigo-600 rounded text-[9px] font-bold text-slate-300 hover:text-white border border-slate-700 transition-all cursor-default"
                  >
          ...
          <main className="flex-1 p-8 bg-white">
            <header className="mb-8">
              <h1 className="text-[34px] font-black text-[#0f172a] leading-tight uppercase tracking-tighter">
                {contactInfo?.name?.split(' ')[0]} <span className="text-[#4f46e5]">{contactInfo?.name?.split(' ').slice(1).join(' ')}</span>
              </h1>
              <p className="text-[15px] font-bold text-[#64748b] uppercase tracking-[0.2em] mt-1">
                {contactInfo?.profession}
              </p>
            </header>
            ...
            {summary && (
              <section className="mb-6">
                <h2 className={sectionHeader}>
                  <span className="w-6 h-[2px] bg-[#4f46e5]"></span>
                  Profile
                </h2>
                <p className="text-[12.5px] leading-relaxed text-[#475569] text-justify">
                  {summary}
                </p>
              </section>
            )}
              ...
              <div className="flex flex-col gap-5">
                {experience?.map((exp, i) => (
                  <div key={i} className="relative pl-5 border-l-2 border-gray-100">
                    <div className="absolute -left-[7px] top-1.5 w-3 h-3 rounded-full bg-[#4f46e5] border-2 border-white"></div>
                    <div className="flex justify-between items-baseline mb-0.5">
                      <h3 className="text-[14px] font-black text-[#1e293b] uppercase tracking-tight">{exp?.title}</h3>
                      <span className="text-[10px] font-black text-[#4f46e5] uppercase bg-indigo-50 px-2 py-0.5 rounded-full">{exp?.startDate} — {exp?.endDate}</span>
                    </div>
                    <div className="text-[12px] font-bold text-[#64748b] mb-1">{exp?.organization}</div>
                    <div className="text-[12.5px] leading-relaxed text-[#475569]">{exp?.description}</div>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="flex flex-col gap-4">
                {projects?.map((proj, i) => (
                  <div key={i} className="p-4 bg-slate-50 rounded-xl border border-slate-100 group hover:border-indigo-200 transition-all">
                    <div className="flex justify-between items-center mb-1.5">
                      <h3 className="text-[14px] font-black text-[#1e293b] uppercase tracking-tight">{proj?.title}</h3>
                      <div className="flex gap-3 text-[10px] font-black text-[#4f46e5] uppercase">
                        {proj?.githubLink && <a href={formatUrl(proj.githubLink)} target="_blank" rel="noopener noreferrer" className="hover:underline">GitHub</a>}
                        {proj?.liveLink && <a href={formatUrl(proj.liveLink)} target="_blank" rel="noopener noreferrer" className="hover:underline">Live</a>}
                      </div>
                    </div>
                    <p className="text-[12px] text-[#64748b] leading-relaxed">{proj?.description}</p>
                  </div>
                ))}
              </div>
            </section>
        ...
        <div className="shadow-[0_0_50px_rgba(0,0,0,0.1)]">
          {mode === 'letter' ? (
            ...
            <>
              ...
              {activeStyle === 'modern' && renderModern()}
            </>
          )}
        </div>
      </div>
